...
class BackupManager:
    ...
    def crear_backup(self) -> bool:
        ...
        try:
            ...
            transacciones = self.firebase_manager.obtener_transacciones()
            ...
            for trans in transacciones:
                ...
                """, (
                    ...
                    trans.get('tipo'),
                    trans.get('equipo_id'),
                    trans.get('cliente_id'),
                    trans.get('operador_id'),
                    self._datetime_to_str(trans.get('fecha')),
                    trans.get('monto'),
                    trans.get('descripcion'),
                    trans.get('comentario'),
                    trans.get('horas'),
                    trans.get('precio_por_hora'),
                    trans.get('conduce'),
                    trans.get('ubicacion'),
                    1 if trans.get('pagado', False) else 0,
                    trans.get('categoria'),
                    trans.get('subcategoria'),
                    self._datetime_to_str(trans.get('fecha_creacion')),
                    self._datetime_to_str(trans.get('fecha_modificacion'))
                ))
                count_transacciones += 1
            
            logger.info(f"Backup de {count_transacciones} transacciones completado")
            ...
            for mant in mantenimientos:
                ...
                """, (
                    mant.get('id'),
                    mant.get('equipo_id'),
                    self._datetime_to_str(mant.get('fecha')),
            ...
            pagos = self.firebase_manager.obtener_pagos_operadores()
            count_pagos = 0
            for pago in pagos:
                cur.execute("""
                ...
                """, (
                    ...
                    pago.get('equipo_id'),
                    self._datetime_to_str(pago.get('fecha')),
                    pago.get('monto'),
                    pago.get('horas'),
