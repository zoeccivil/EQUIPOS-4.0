...
class RegistroAlquileresTab(QWidget):
    ...
    def _accion_ver_conduce(self):
        """Intenta abrir el conduce del alquiler seleccionado (URL directa o firmada)."""
        alquiler_id = self._obtener_id_seleccionado()
        if not alquiler_id:
            return
        try:
            doc = self.fm.obtener_alquiler_por_id(alquiler_id) or {}
            # Prioridad: URL directa guardada
            url = (doc.get("conduce_url") or doc.get("conduceUrl") or "").strip()
            if url:
                self._abrir_url(url)
                return

            # Si no hay URL, intentar firmar un path en Storage
            storage_path = (doc.get("conduce_storage_path") or doc.get("conducePath") or "").strip()
            if not storage_path:
                QMessageBox.information(self, "Conduce", "Este registro no tiene archivo de conduce adjunto.")
                return

            if not self.sm:
                QMessageBox.warning(self, "Conduce", "Storage no está configurado en esta instalación.")
                return

            signed = self._generar_url_firmada(storage_path, dias=7)
            if not signed:
                QMessageBox.warning(self, "Conduce", "No fue posible generar la URL firmada del conduce.")
                return

            self._abrir_url(signed)
        except Exception as e:
            logger.error(f"Error al abrir conduce de {alquiler_id}: {e}", exc_info=True)
